...
export interface IProductModel extends IBase<Product>{
    ...
    updateAmount(prod:Pick<Product,"id"|"amount"|"price">,trx?:Knex.Transaction):Promise<Boolean>;
}

export class ProductModel extends Base<Product> implements IProductModel{
   tableName = "products"; 
    // key->product object,value -> DB object
    schema = {
        id: "id",
        name: "name",
        amount: "amount",
   ...
   public updateAmount = async (product:Pick<Product,"id"|"amount"|"price">,trx?:Knex.Transaction) => {
        let queryBuilder = this.knexSql(this.tableName)
            ...
            .update( this.schema.amount,this.knexSql.raw(`??-?`,[this.schema.amount,product.amount]))
            .update( this.schema.preOrder,this.knexSql.raw(`??-?`,[this.schema.preOrder,product.amount]));
